...
import { BadgeDollarSign, CircleAlert, ExternalLink, LoaderCircle, RefreshCw } from 'lucide-vue-next'
import { t } from '../i18n'
import { readCachedPrices, writeCachedPrices } from '../services/domainCache'
...
<template>
  <section class="price-comparison" :aria-busy="loading" :aria-label="t('prices.title')">
    <div class="price-title">
      <BadgeDollarSign :size="18" /><strong>{{ t('prices.title') }}</strong><span>{{ domain }}</span>
      <button type="button" class="icon-button price-refresh" :disabled="loading" :title="t('prices.refresh')" :aria-label="t('prices.refreshAria', { name: domain })" @click="loadPrices({ forceRefresh: true })"><RefreshCw class="refresh-icon" :class="{ spin: loading }" :size="15" /></button>
    </div>
    ...
    <template v-else>
      <div v-if="quotes.length" class="quote-table" role="table" :aria-label="t('prices.title')">
        ...
        <div v-for="quote in pricedQuotes" :key="quote.registrar" class="quote-row" role="row">
          ...
          <span class="quote-link-cell" role="cell"><a :href="quote.url" target="_blank" rel="noreferrer" :aria-label="t('prices.openRegistrarAria', { registrar: quote.registrar, name: domain })"><ExternalLink :size="16" /></a></span>
        </div>
