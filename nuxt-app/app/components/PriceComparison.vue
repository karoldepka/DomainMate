...
import { getRegistrarLinks } from '../services/registrarLinks.js'
...
<template>
  <section class="price-comparison" :aria-busy="loading" :aria-label="t('prices.title')">
    <div class="price-title"><UIcon name="i-lucide-badge-dollar-sign" class="size-4.5" /><strong>{{ t('prices.title') }}</strong><span>{{ domain }}</span></div>
    ...
    <template v-else>
      ...
      <div v-for="(quote, index) in pricedQuotes" :key="quote.registrar" class="quote-row">
        ...
        <a :href="quote.url" target="_blank" rel="noreferrer" :aria-label="`${quote.registrar}: ${domain}`"><UIcon name="i-lucide-external-link" class="size-4" /></a>
      </div>
      ...
      <ul class="registrar-link-grid" role="list">
        <li v-for="registrar in registrarLinks" :key="registrar.name">
          <a :href="registrar.url" target="_blank" rel="noopener noreferrer" :aria-label="t('prices.openRegistrarAria', { registrar: registrar.name, name: domain })">
            <span>{{ registrar.name }}</span><UIcon name="i-lucide-external-link" class="size-3.75" aria-hidden="true" />
          </a>
        </li>
      </ul>
    </div>
