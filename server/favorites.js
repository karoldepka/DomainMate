...
`)
const selectByClient = database.prepare('SELECT domain, rating, updated_at FROM favorites WHERE client_id = ? ORDER BY updated_at DESC')
 ...
 */
export function syncFavorites(clientId, records) {
  ...
  return selectByClient.all(clientId).map((record) => ({
...
/** Migrate the pre-rating schema (boolean `starred` column) to the 1-5 `rating` scale once. */
function migrateStarredColumn() {
  ...
  try { database.exec('ALTER TABLE favorites DROP COLUMN starred') } catch { /* Older SQLite builds keep the unused column. */ }
}
